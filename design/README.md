...
```mermaid
graph TD
    UI[UI Layer (Flutter)] -->|User Action| Logic[Logic Layer]
    Logic -->|Data Request| Repository[Repository Layer]
    Repository -->|Local Storage| SQLite[SQLite Database]
    Repository -->|Remote Sync| API[Remote API (Python + PostgreSQL)]
```
